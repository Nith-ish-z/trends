...
export default function AddProduct({prdData}) {
    ...
    const handleRefImg = (ind,val) => {
        ...
        prev[ind] = val
        setForm((p)=> ({...p,['imageUrl']:prev}));
    };

    const handleAdd = () => {
        const newImg = [...form.imageUrl,''];
        setForm((p)=> ({...p,['imageUrl']:newImg}));
        setNewInputIndex(form.imageUrl.length);
    };

    const handleDelete = (ind) => {
        form.imageUrl.length > 1 ?
        setForm((p)=> ({...p,['imageUrl']: form.imageUrl.filter(( _,i) => i !== ind)}))
